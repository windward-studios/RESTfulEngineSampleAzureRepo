...
namespace AzureRepositoryPlugin
{
    public class AzureStorageManager
    {
        ...
        private readonly string _jobInfoConnectionString;
        private readonly string _jobBlobStorageConnectionString;
        ...
        public AzureStorageManager(ICloudTableFactory tableFactory, IBlobContainerFactory blobFactory)
        {
            ...
            _jobInfoConnectionString = ConfigurationManager.AppSettings["AzureRepository:TableStorageAccountConnectionString"];
            _jobBlobStorageConnectionString = ConfigurationManager.AppSettings["AzureRepository:BlobStorageAccountConnectionString"];
        }

        public async Task Init()
        {
            _jobInfoTable = await _tableFactory.CreateCloudTableWrapper<JobInfoEntity>(_jobInfoConnectionString, JOB_INFO_TABLE_NAME);
            _jobBlobContainer = await _blobFactory.CreateBlobContainerWrapper(_jobBlobStorageConnectionString, JOB_BLOB_NAME);
        }
